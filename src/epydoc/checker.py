...
class DocChecker:
    ...
    def __init__(self, docindex):
        ...
        # Sort & filter the documentation from the docindex.
        docs = docindex.items()
        docs.sort()
        self._docs = [d for (u,d) in docs if u.is_module() or
                      docindex.has_key(u.module())]
    ...
    def _check_module(self, doc):
        ...
        @rtype: C{None}
        """
        if not self._check_name_publicity(doc.canonical_name): return
        if self._checks & DocChecker.MODULE:
            self._check_basic(doc)
        if self._checks & DocChecker.VAR:
            for v in doc.variables():
                self._check_var(v, doc.canonical_name)
    ...
    def _check_func(self, doc):
        ...
        if (doc.uid().is_any_method() and 
            doc != self._docindex.documented_ancestor(doc.uid())): return
