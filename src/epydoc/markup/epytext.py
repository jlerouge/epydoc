...
def parse(str, errors = None, warnings = None):
    ...
    @param errors: A list where any errors generated during parsing
        ...
        generate exceptions.
    @type errors: C{list} of C{ParseError}
...
def _tokenize_doctest(lines, start, block_indent, tokens, warnings):
    """
    Construct a C{Token} containing the doctest block starting at
...
def _tokenize_para(lines, start, para_indent, tokens, warnings):
    """
    Construct a C{Token} containing the paragraph starting at
...
def to_html(tree, indent=0, seclevel=0, **kwargs):
    ...
    string does not include C{<HTML>} and C{</HTML>} tags.

    todo: doesn't honor ordered list bullets.
    """
    if isinstance(tree, Text): return tree.toxml()
...
def to_plaintext(tree, indent=0, seclevel=0):
    """
    Given the DOM tree for an epytext string (as
    returned by C{parse()}), return a string encoding it in plaintext.
    This function is similar to C{to_epytext()}; however, it prints
    ...
    if tree.tagName == 'para':
        ...
        return ((indent-2)*' ' + childstr + '\n' +
                (indent-2)*' ' + uline + '\n')
    elif tree.tagName in ('doctestblock', 'literalblock'):
...
def to_debug(tree, indent=4, seclevel=0):
    ...
    if tree.tagName == 'para':
        ...
        return 'LIST>|'+(indent-6)*' '+childstr[indent:]
    elif tree.tagName == 'field':
        if (len(tree.childNodes) > 1 and
            tree.childNodes[1].tagName == 'arg'):
