...
class HTML_Doc:
    ...
    def _project_toc_to_html(self):
        ...
        classes = [Link(`c`,c) for c in self._docmap.keys() if c.is_class()]
    ...
    def _navbar(self, where=None, top=0):
        ...
        if self._module is None: pass
            ...
            str += I+'<th '+WIDTH+' class="navbar">&nbsp;&nbsp;&nbsp;'
            str += '<a class="navbar" href="'+`where.module()`+'.html">'
            str += 'Module</a>&nbsp;&nbsp;&nbsp;</th>\n'
        elif isinstance(self._module, UID):
            str += I+'<th '+WIDTH+' class="navbar">&nbsp;&nbsp;&nbsp;'
    ...
    def _toc_var_section(self, section, vars):
        ...
        if not self._show_private:
            vars = [o for o in vars if not self._is_private(o[0].name())]
        ...
        for var in vars:
            str += ('<a target="mainFrame" href="%s#%s">%s</a><br>\n' %
                    (self._uid_to_uri(var[1]), var[0].name(), var[0].name()))
        return str+'<br>\n'
