...
class DocstringParser:
    ...
    def parse_docstring(self, api_doc):
        ...
        if isinstance(api_doc, RoutineDoc):
            self.parse_function_signature(api_doc)
    ...
    def postprocess_api_doc(self, api_doc, field_warnings):
        # Extract a summary
    ...
    def process_field(self, api_doc, tag, arg, descr):
        ...
        if tag in self.FIELD_DISPATCH_TABLE:
            handler = getattr(self, self.FIELD_DISPATCH_TABLE[tag])
        ...
        for field in self.STANDARD_FIELDS:
            if tag in field.tags:            
                ...
                if not field.takes_arg:
                    self.check(api_doc, tag, arg, expect_arg=False)
                ...
                if not field.multivalue and len(values) > 0:
                    raise ValueError(self.REDEFINED % tag)
                values.append(descr)
                return


    # [xx] use 'FOO%locals()' and '%(tag)s' etc?
    ...
    #: Maps tag -> method name.
    FIELD_DISPATCH_TABLE = {
        'group':        'process_group_field',
        'deffield':     'process_deffield_field',
          'newfield':   'process_deffield_field',
        'sort':         'process_sort_field',
        'summary':      'process_summary_field',
        'undocumented': 'process_undocumented_field',
        'include':      'process_include_field',
        'var':          'process_var_field',
          'variable':   'process_var_field',
        'type':         'process_type_field',
        'cvar':         'process_cvar_field',
          'cvariable':  'process_cvar_field',
        'ivar':         'process_ivar_field',
          'ivariable':  'process_ivar_field',
        'return':       'process_return_field',
          'returns':    'process_return_field',
        'rtype':        'process_rtype_field',
          'returntype': 'process_rtype_field',
        'arg':          'process_arg_field',
          'argument':   'process_arg_field',
          'parameter':  'process_arg_field',
          'param':      'process_arg_field',
        'kwarg':        'process_kwarg_field',
          'keyword':    'process_kwarg_field',
          'kwparam':    'process_kwarg_field',
        'raise':        'process_raise_field',
          'raises':     'process_raise_field',
          'except':     'process_raise_field',
          'exception':  'process_raise_field',
        }

    def process_summary_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, expect_arg=False)
        if api_doc.summary is not None:
            raise ValueError(self.REDEFINED % tag)
        api_doc.summary = descr

    def process_include_field(self, api_doc, tag, arg, descr):
        # [xx] need docindex!!
        self.check(api_doc, tag, arg, expect_arg=False)
        # options:
    ...
    def process_undocumented_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=NamespaceDoc, expect_arg=False)
    ...
    def process_deffield_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, expect_arg=True)
    ...
    def process_raise_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=RoutineDoc, expect_arg='single')
        try: name = DottedName(arg)
        except ValueError: name = arg
        api_doc.exception_descrs.append( (name, descr) )

    def process_todo_field(self, api_doc, tag, arg, descr):
        raise ValueError('not implemented yet?')

    def process_sort_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=NamespaceDoc, expect_arg=False)
        api_doc.sort_spec += _descr_to_identifiers(descr)
        
    def process_type_field(self, api_doc, tag, arg, descr):
        # In namespace, "@type var: ..." describes the type of a var.
        if isinstance(api_doc, NamespaceDoc):
            self.check(api_doc, tag, arg, expect_arg='single')
            self.set_var_type(api_doc, arg, descr)
        ...
        # For routines, "@type param: ..." describes a parameter.
            ...
            if arg in api_doc.arg_types:
                raise ValueError(self.REDEFINED % ('type for '+arg))
            api_doc.arg_types[arg] = descr
        else:
            raise ValueError(BAD_CONTEXT % arg)

    def process_var_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=ModuleDoc, expect_arg=True)
        for ident in re.split('[:;, ] *', arg):
            self.set_var_descr(api_doc, ident, descr)
            
    def process_cvar_field(self, api_doc, tag, arg, descr):
    ...
    def process_return_field(self, api_doc, tag, arg, descr):
        ...
        if api_doc.return_descr is not None:
            raise ValueError(self.REDEFINED % 'return value description')
        api_doc.return_descr = descr

    def process_rtype_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=RoutineDoc, expect_arg=False)
        if api_doc.return_type is not None:
            raise ValueError(self.REDEFINED % 'return value description')
        api_doc.return_type = descr

    def process_arg_field(self, api_doc, tag, arg, descr):
        self.check(api_doc, tag, arg, context=RoutineDoc, expect_arg=True)
        idents = re.split('[:;, ] *', arg)
        api_doc.arg_descrs.append( (idents, descr) )

    def process_kwarg_field(self, api_doc, tag, arg, descr):
    ...
    def check(self, api_doc, tag, arg, context=None, expect_arg=None):
        if context is not None:
            if not isinstance(api_doc, context):
                raise ValueError(self.BAD_CONTEXT % tag)
        if expect_arg is not None:
            if expect_arg == True:
                if arg is None:
                    raise ValueError(self.EXPECTED_ARG % tag)
            elif expect_arg == False:
                if arg is not None:
                    raise ValueError(self.UNEXPECTED_ARG % tag)
            elif expect_arg == 'single':
                if (arg is None or ' ' in arg):
                    raise ValueError(self.EXPECTED_SINGLE_ARG % tag)
            else:
    ...
    def unindent_docstring(self, docstring):
    ...
    def parse_function_signature(self, func_doc):
        """
        ...
        m = self._SIGNATURE_RE.match(func_doc.docstring)
